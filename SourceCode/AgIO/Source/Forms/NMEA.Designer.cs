...
namespace AgIO
{
    public partial class FormLoop
    {
        ...
        private string rawBuffer2 = "";
        private string[] words2;
        private string nextNMEASentence2 = "";
        private bool isNMEAToSend2 = false;
        ...
        public byte fixQualityData2, fixQuality2 = byte.MaxValue;
        ...
        private void ParseVTG()
        {
            ...
            if (lastSentence == "VTG") isNMEAToSend2 = true;
        }
        ...
        //Parse Tool GPS
        public void ParseNMEA2(ref string buffer2)
        {
            if (rawBuffer2 == null) return;

            //find end of a sentence
            int cr = rawBuffer2.IndexOf("\r", StringComparison.Ordinal);
            if (cr == -1) return; // No end found, wait for more data

            // Find start of next sentence
            int dollar = rawBuffer2.IndexOf("$", StringComparison.Ordinal);
            if (dollar == -1) return;

            //if the $ isn't first, get rid of the tail of corrupt sentence
            if (dollar >= cr) rawBuffer2 = rawBuffer2.Substring(dollar);

            cr = rawBuffer2.IndexOf("\r", StringComparison.Ordinal);
            if (cr == -1) return; // No end found, wait for more data
            dollar = rawBuffer2.IndexOf("$", StringComparison.Ordinal);
            if (dollar == -1) return;
            ...
            if (rawBuffer2.Length > 301)
            {
                if (isLogNMEA)
                {
                    logNMEASentence2.Append("\r\n" +
                        DateTime.UtcNow.ToString(" ->>  mm:ss.fff ", CultureInfo.InvariantCulture) + "\r\n" + rawBuffer2 + "\r\n");
                }
            ...
            {
                //extract the next NMEA single sentence
                nextNMEASentence2 = Parse2(ref buffer2);
                if (nextNMEASentence2 == null) break;

                words2 = nextNMEASentence2.Split(',');

                if (isLogNMEA)
                {
                    string timNow = DateTime.UtcNow.ToString("HHmmss.fff ", CultureInfo.InvariantCulture);
                    logNMEASentence2.Append(timNow + " " + nextNMEASentence2 + "\r\n");
                }

                //parse them accordingly
                if (words2.Length < 3) break;

                if ((words2[0] == "$GPGGA" || words2[0] == "$GNGGA") && words2.Length > 13)
                {
                    ParseGGA2();
                    //if (isGPSSentencesOn) ggaSentence = nextNMEASentence2;
                }

                else if (words2[0] == "$PANDA" && words2.Length > 14)
                {
                    ParsePANDA2();
                ...
                //{
                //    ParseVTG();
                //    //if (isGPSSentencesOn) vtgSentence = nextNMEASentence2;
                //}
            ...
            if (isNMEAToSend2)
            {
                ...
                longitudeSend2 = double.MaxValue;

                //latitude
                Buffer.BlockCopy(BitConverter.GetBytes(latitudeSend2), 0, nmeaPGN, 13, 8);
                latitudeSend2 = double.MaxValue;
        ...
        public string Parse2(ref string buffer)
        {
            string sentence;
            do
            {
        ...
        private void ParseGGA2()
        {
                ...
                && !string.IsNullOrEmpty(words2[4]) && !string.IsNullOrEmpty(words2[5]))
            {
                ...
                longitude2 += temp * 0.0166666666667;
                ...
                //age
                float.TryParse(words2[13], NumberStyles.Float, CultureInfo.InvariantCulture, out ageData2);
                age2X100 = (ushort)(ageData2 * 100.0);

                if (lastSentence == "GGA") isNMEAToSend2 = true;
                //}
            }
        }

        private void ParsePANDA2()
        {
            #region PANDA Message
            /*
            $PANDA
